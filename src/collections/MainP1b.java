...
class TestStackSize extends STest<Long> {
	public boolean exec(Stack<Long> stack, Object... args) {
		...
		return MainP1b.test(stack.size()==i, "stack.size()=="+i, "stack.size()!="+i);
	}
}

class TestStackIsEmpty extends STest<Long> {
	public boolean exec(Stack<Long> stack) {
		return MainP1b.test(stack.isEmpty(), "stack.isEmpty()", "stack.isEmpty()");
	}
}
...
class TestPushPeekPop extends STest<Long> {
	public boolean exec(Stack<Long> stack, Object... args) {
		...
		boolean res = MainP1b.method(false, true, stack, new TestStackSize(),
				args.length);
		for (int i = 0; i < args.length; i++) {
			res = MainP1b.test(stack.peek().equals(args[args.length - 1 - i]),
					"stack.peek()", "stack.peek()") && res;
			res = MainP1b.test(stack.pop().equals(args[args.length - 1 - i]),
					"stack.pop()", "stack.pop()") && res;
		}
		res = MainP1b.method(false, true, stack, new TestStackSize(), 0) && res;
		res = MainP1b.method(false, true, stack, new TestStackIsEmpty()) && res;
		try {
			for (long i = 0; i < 20; i++) {
				stack.push(i);
			}
...
class TestListSize extends LTest<Long> {
	public boolean exec(List<Long> list, Object... args) {
		Integer i = (Integer)args[0];
		return MainP1b.test(list.size()==i, "list.size()=="+i, "list.size()!="+i);
	}
}
...
class TestGet extends LTest<Long> {
	public boolean exec(List<Long> list, Object... args) {
		...
		return MainP1b.test(res, "list.get()","list.get()");
	}
}
...
class TestSetIndexOf extends LTest<Long> {
	public boolean exec(List<Long> list) {
		...
		res = MainP1b.test(list.indexOf(3L)==1, "list.indexOf(value)","list.indexOf(value)") && res;
		res = MainP1b.test(list.indexOf(-1L)==-1, "list.indexOf(value)","list.indexOf(value)") && res;
		res = MainP1b.test(list.indexOf(2,3L)==3, "list.indexOf(index,value)","list.indexOf(index,value)") && res;
		list.set(0, Long.MAX_VALUE);
		list.set(1, Long.MIN_VALUE);
		list.set(3, Long.MIN_VALUE);
		res = MainP1b.test(list.get(0).equals(Long.MAX_VALUE) && list.get(1).equals(Long.MIN_VALUE), "list.set()","list.set()") && res;
		return res;
	}
}
...
class TestClear extends LTest<Long> {
	public boolean exec(List<Long> list) {
		list.clear();
		return MainP1b.test(list.size()==0, "list.clear()", "list.clear()");
	}
}
...
class TestAdd extends LTest<Long> {
	public boolean exec(List<Long> list, Object... args) {
		...
		MainP1b.method(false,true,list,new TestListSize(), args.length);
		MainP1b.method(false, true, list, new TestGet(), arr1);
		return MainP1b.test(MainP1b.equals(list,arr2), "list.add(elem)", "list.add(elem)");
	}
}
...
class TestAddLast extends LTest<Long> {
	public boolean exec(List<Long> list, Object... args) {
		...
		MainP1b.method(false,true,list,new TestListSize(), args.length);
		return MainP1b.test(MainP1b.equals(list,arr), "list.addLast(elem)", "list.addLast(elem)");
	}
}
...
class TestAddFirst extends LTest<Long> {
	public boolean exec(List<Long> list, Object... args) {
		...
		MainP1b.method(false,true,list,new TestListSize(), args.length);
		return MainP1b.test(MainP1b.equals(list,arr), "list.addFirst(elem)", "list.addFirst(elem)");
	}
}
...
class TestAddIndex extends LTest<Long> {
	public boolean exec(List<Long> list) {
		...
		MainP1b.method(false,true,list,new TestListSize(), 5);
		return MainP1b.test(MainP1b.equals(list,arr2), "list.add(index,elem)", "list.add(index,elem)");
	}
}
...
class TestIterator extends LTest<Long> {
	public boolean exec(List<Long> list) {
        ...
        return MainP1b.test(res, "list.iterator()", "list.iterator()");
	}
}
...
class TestRemoveLast extends LTest<Long> {
	public boolean exec(List<Long> list) {
		...
		list.removeLast();
		MainP1b.method(false,true,list,new TestListSize(), 5);
		return MainP1b.test(MainP1b.equals(list,arr), "list.removeLast()", "list.removeLast()");
	}
}
...
class TestRemoveFirst extends LTest<Long> {
	public boolean exec(List<Long> list) {
		...
		list.removeFirst();
		MainP1b.method(false,true,list,new TestListSize(), 5);
		return MainP1b.test(MainP1b.equals(list,arr), "list.removeFirst()", "list.removeFirst()");
	}
}
...
class TestRemoveIndex extends LTest<Long> {
	public boolean exec(List<Long> list) {
		...
		list.remove(2);
		MainP1b.method(false,true,list,new TestListSize(), 5);
		return MainP1b.test(MainP1b.equals(list,arr), "list.remove(index)", "list.remove(index)");
	}
}
